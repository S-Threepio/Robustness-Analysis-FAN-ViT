...
model_path = "./pretrained_models/fan_hybrid_tiny.pth.tar"
...
# load model
model = create_model("fan_tiny_8_p4_hybrid", img_size=224).to(device)
...
def baselineViT(device, val_data_loader):
    ...
    for val_batch, val_label in tqdm(val_data_loader):
        ...
        val_batch = val_batch.to(device)
        val_label = val_label.to(device)
        pred = baselineModel(val_batch)
        prediction = torch.argmax(pred[0].cpu()).numpy()
        label = val_label.cpu().numpy()[0]
        if prediction == label:
            accuracy += 1
    print(f"Baseline Model accuracy = {(accuracy/5000)*100}")

    acc_FGSM, ex_FGSM = testFGSM(baselineModel, device, val_data_loader, epsilon)

    for steps in [1, 2, 5, 10]:
        acc_PGD, ex_PGD = testPGD(
            baselineModel, device, val_data_loader, epsilon, steps
        )

    for steps in [1, 2, 5, 10]:
        acc_BIM, ex_BIM = testBIM(
            baselineModel, device, val_data_loader, epsilon, steps
        )

    acc_CW, ex_CW = testCW(baselineModel, device, val_data_loader, epsilon, 50)

    acc_Mim, ex_Mim = testMiM(baselineModel, device, val_data_loader, epsilon, 10)
